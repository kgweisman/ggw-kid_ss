...
# Pilot
pilot_files <- dir("pilot data/individual sessions")
wd <- getwd()

d <- data.frame()
for(i in 1:length(pilot_files)) {
  temp <- read.csv(paste0(wd, "/pilot data/individual sessions/", 
                          pilot_files[i])) %>%
...
write.csv(d, "/Users/kweisman/Documents/Research (Stanford)/Projects/GGW-kid_ss/pilot data/pilot_data_2016-03-23.csv")
